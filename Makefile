...
.envs: .conda .channels
	export PATH=${PBASE}/bin/bin:${PATH} && conda create -y --prefix=${PBASE}/bin/envs/atac samtools=1.7 openssl=1.0 igvtools=2.3.93 alfred=0.1.17 idr=2.0.3 cutadapt=1.16 fastqc=0.11.7 bcftools=1.7 bowtie2=2.3.4.1 bbmap=37.90 biobambam=2.0.87 bedtools=2.27.1 jalview=2.10.3 datamash=1.1.0 && conda install -y --prefix=${PBASE}/bin/envs/atac -c conda-forge ncurses=5.9 && conda create -y --prefix=${PBASE}/bin/envs/atac2 macs2=2.1.1.20160309 homer=4.9.1 samtools=1.7 openssl=1.0 && touch .envs
